import axios from "axios";
import React, { useState } from "react";
import { API_PATH } from "../lib/API_PATH";
import toast from "react-hot-toast";
import { store } from "../lib/store";
import { zodResolver } from "@hookform/resolvers/zod";
import z from "zod";
import { useForm } from "react-hook-form";
import { UpdateUserShema } from "../models/user";

function UserDetails({ currentUser }: any) {
  const { getUserInfo } = store()
  const [firstName, setFirstName] = useState(currentUser?.firstName || "");
  const [lastName, setLastName] = useState(currentUser?.lastName || "");
  const [email, setEmail] = useState(currentUser?.email || "");
  const {
    register,
    handleSubmit,
    formState: { errors },
    setError,
  } = useForm<z.infer<typeof UpdateUserShema>>({
    resolver: zodResolver(UpdateUserShema),
    defaultValues: {
      firstName: "",
      lastName: "",
      email: ""

    }
  })
  const handleSubmit1 = async () => {
    const res = await axios.post(API_PATH.UPDATE_USER_INFO,
      { id: currentUser.id, firstName, lastName, email }
    )
    if (res.data.success) {
      toast.success("kullanıcı bilgilri guncellendi")
      getUserInfo()
    } else {
      toast.error("guncellerinirken bir hata olustu:", res.data.error)
    }
  }
  return (
    <div className=" max-w-xl p-8">
      <h2 className="text-xl font-semibold mb-6 border-b pb-2">Profil bilgileri</h2>

      <div className="mb-5 ">
        <label className="block mb-1 text-sm  ">Ad</label>
        <input
          type="text"
          // value={firstName}
          // onChange={(e) => setFirstName(e.target.value)}
          {...register("firstName")}
          className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm bg-gray-100"
          placeholder="Adınız"
        />
        {errors && <span className="error-message">{errors.firstName?.message}</span>}

      </div>

      <div className="mb-5">
        <label className="block mb-1 text-sm  ">Soyad</label>
        <input
          type="text"
          // value={lastName}
          // onChange={(e) => setLastName(e.target.value)}
          {...register("lastName")}

          className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm bg-gray-100"
          placeholder="Soyadınız"
        />
        {errors && <span className="error-message">{errors.lastName?.message}</span>}

      </div>

      <div className="mb-5">
        <label className="block mb-1 text-sm  ">Email</label>
        <input
          type="email"
          // value={email}
          // onChange={(e) => setEmail(e.target.value)}
          {...register("email")}
          className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm bg-gray-100"
          placeholder="E-posta adresiniz"
        />
        {errors && <span className="error-message">{errors.email?.message}</span>}

      </div>

      <button

        className="bg-orange-600 hover:bg-orange-700 mt-4 px-6 py-2 rounded-md font-semibold"
      >
        Güncelle
      </button>
    </div>
  );
}

export default UserDetails;
