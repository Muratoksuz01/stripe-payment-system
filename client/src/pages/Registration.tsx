...
import { RegisterShema } from "../models/user";
...
export default function Registration() {
  ...
  const onSubmit = async (data: any) => {
    try {
      const res = await axios.post(API_PATH.register, data);
      if (res.data.success) {
        alert("giriş yapıldı");
  ...
  return (
    <div className="bg-gray-950 rounded-lg">
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-5xl mx-auto pt-10 px-10 lg:px-0 text-white">
          ...
          <div className="mt-5 grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-6">
            <div className="sm:col-span-3">
              ...
              <input type="text"
                ...
                {...register("firstName")}
                className="block w-full rounded-md bg-white/5 py-1.5 px-4 mt-2 outline-none ring-1 ring-white/10" />
                {errors && <span className="error-message">{errors.firstName?.message}</span>}
            </div>
            ...
            <div className="sm:col-span-3">
              ...
              <input type="text"
                ...
                {...register("lastname")}
                className="block w-full rounded-md bg-white/5 py-1.5 px-4 mt-2 outline-none ring-1 ring-white/10" />
                {errors && <span className="error-message">{errors.lastname?.message}</span>}
            </div>
